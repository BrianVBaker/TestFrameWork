...
namespace BatchRunner
{
    public class RunTesting
    {
            ...
            foreach (string TestStep in TestSteps)
            {
                ...
                if (TestRun.TestStep != "Close")
                {
                    if (!teststatus)
                    {
                        ...
                        if (TestRun.ExpectedResult == "Fail")
                        {
                            ...
                            if (!Results.ContainsKey(TestRun.TestStep))
                            {
                                ...
                                log.Info("Test Step '" + TestRun.TestStep + "' aleady encountered in test set. Possible duplicate Test step");
                        ...
                        else
                        {
                            ...
                            if (!Results.ContainsKey(TestRun.TestStep))
                                ...
                                log.Error("Test Step '" + TestRun.TestStep + "' aleady encountered in test set. Possible duplicate Test step");
                    ...
                    else
                    {
                        if (TestRun.ExpectedResult == "Fail")
                        {
                            log.Warn("test Step " + TestRun.TestStep + " has completed without errors, which was not expected! (warning)");
                            if (!Results.ContainsKey(TestRun.TestStep))
                            {
                                Results.Add(TestRun.TestStep, false);
                            }
                        ...
                        {
                            log.Info("test Step " + TestRun.TestStep + " has completed successfuly!");
                            if (!Results.ContainsKey(TestRun.TestStep))
                                Results.Add(TestRun.TestStep, true);
                            else
                                log.Warn("Test Step '" + TestRun.TestStep + "' aleady encountered in test set. Possible duplicate Test step");
                ...
                if (!overallPass)
                {
                    log.Fatal("Test Set has (real) Errors!!");
        ...
        public static bool RunStep(string test, string dep, string Type, string Page, string URL, string load, string reload, string Elem, By str, string disp, string act, string val, string Exp, int pause, int waitParm)
        {
            ...
            if (disp == "Yes")
            {
                if (!SeleniumActions.TestObjectExists(str, timer, Elem, TestRun.Driver))
                {
            ...
            switch (act)
            {
                ...
                case "Evaluate":
                    Rtn = CompareValues(val, Exp);                                           
                    break;

                case "Enter":
                    Rtn = SeleniumActions.Enter(str, TestRun.Driver, Config.Time, val);
                    break;

                case "Escape":
                    Rtn = SeleniumActions.Escape(str, TestRun.Driver, Config.Time, val);
                    break;
        ...
        public static bool CompareValues(string val, string exp)
        {
            bool Rtn = false;
            string[] parms = val.Split(',');
            string op = parms[1];  string value = parms[0];
            switch (op)
            {
                case "EQ":
                    if (value == exp)
                    ...
                    break;
                case "GT":
                    if (Convert.ToInt32(value) > Convert.ToInt32(exp))
                        Rtn = true;
                    break;
                case "LT":
                    if(Convert.ToInt32(value) < Convert.ToInt32(exp))
            ...
            if (Rtn)
                log.Info("Compare passed! " + value + " found to be " + op + " " + exp);
            else
                log.Error("Compare failed!! " + value + " NOT found to be " + op + " " + exp);
